function output_Img = inverseGaussianBlur(input_Img, sigma)
if nargin < 2
    sigma = 1;
end
...
filter_buffer = filter_buffer /correction; % correction is used to ensure that sum of filter = 1
%correction
%filter_buffer
%size(filter_buffer)
...
filter_buffer_inv = ifft((1/fft(filter_buffer,size(input_Img,1))));
...
for i=1:size(filter_buffer_inv,1)
    for j=1:size(filter_buffer_inv,2)
        correction = correction +filter_buffer_inv(i,j);
    end
end
filter_buffer_inv = filter_buffer_inv /correction;%normalization done to preserve avg. brightness
...
for i=1:size(input_Img,1)
    for j=1:size(input_Img,2)
        ...
        for b = -filter_range:filter_range
            ...
            total = total +filter_buffer_inv(filter_range-b+1) * input_Img_double(i,y) ;               
        end
...
for i=1:size(input_Img,1)
    for j=1:size(input_Img,2)
        ...
        for a = -filter_range:filter_range
            ...
            output_Img(i,j) = output_Img(i,j) + filter_buffer_inv(filter_range-a+1) * output_Img_temp(x,j) ;  
        end
...
output_Img = (output_Img);%uint8(output_Img);
end
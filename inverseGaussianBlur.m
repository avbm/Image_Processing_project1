function output_Img = inverseGaussianBlur(input_Img, sigma=1);
...
for i=1:size(input_Img,1)
    for j=1:size(input_Img,2)
        ...
        for b = -filter_range:filter_range
            ...
            total += filter_buffer_inv(filter_range-b+1) * input_Img_double(i,y) ;               
        endfor
...
for i=1:size(input_Img,1)
    for j=1:size(input_Img,2)
        ...
        for a = -filter_range:filter_range
            ...
            total += filter_buffer_inv(filter_range-a+1) * output_Img_temp(x,j) ;  
        endfor
